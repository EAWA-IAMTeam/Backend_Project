...
func NewRequestHandler(nc *nats.Conn, js nats.JetStreamContext) *RequestHandler {
	...
	_, err := js.AddStream(&nats.StreamConfig{
		Name:     "store",
		Subjects: []string{"store.request.*", "store.response.*"},
	...
	_, err = js.AddConsumer("store", &nats.ConsumerConfig{
		Durable:       "store-replies",
		FilterSubject: "store.response.*", // Listen to all responses
